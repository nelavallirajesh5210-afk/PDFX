...
export const PdfToWordTool: React.FC<PdfToWordToolProps> = ({ onBack }) => {
  ...
  const handleConvert = async () => {
    ...
    setProcessing({ status: 'uploading', progress: 25, stepMessage: 'Uploading PDF...' });

    try {
      const res = await convertPdfToWord(file, (step, percent) => {
      ...
      setProcessing({
        ...
        progress: 0,
        stepMessage: '',
        errorMessage: err.message || 'Failed to convert PDF to Word.',
      });
    }
  };
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-blue-50 text-blue-600 mb-3">
          <FileCode2 className="w-7 h-7 stroke-[2.2]" />
        </div>
      ...
      {result ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center shadow-sm space-y-6">
          ...
          <div className="space-y-1">
            <h2 className="text-2xl font-bold text-slate-900">Your Word document is ready!</h2>
            <p className="text-sm text-slate-600">
              Easily open and edit your document text in Microsoft Word or Google Docs.
            </p>
          </div>
